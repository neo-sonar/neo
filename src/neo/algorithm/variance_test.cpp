...
TEMPLATE_TEST_CASE("neo/algorithm: variance", "", float, double)
{
    ...
    SECTION("vector")
    {
        ...
        REQUIRE_FALSE(neo::variance(size_0.to_mdspan()).has_value());
        REQUIRE_FALSE(neo::variance(size_1.to_mdspan()).has_value());

        REQUIRE(neo::variance(zeros.to_mdspan()).has_value());
        REQUIRE_THAT(neo::variance(zeros.to_mdspan()).value(), Catch::Matchers::WithinAbs(0.0, 0.000001));

        REQUIRE(neo::variance(ones.to_mdspan()).has_value());
        REQUIRE_THAT(neo::variance(ones.to_mdspan()).value(), Catch::Matchers::WithinAbs(0.0, 0.000001));
        ...
        REQUIRE(neo::variance(values).has_value());
        REQUIRE_THAT(neo::variance(values).value(), Catch::Matchers::WithinAbs(4.0, 0.000001));
    }
